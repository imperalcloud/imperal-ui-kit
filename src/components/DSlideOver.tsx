...
export const DSlideOver: UIComponent = ({ node, onAction }) => {
  ...
  return (
    <div className="fixed inset-0 z-[var(--imp-z-modal)] flex justify-end" role="presentation">
      ...
      <aside ref={panelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={title ? titleId : undefined} aria-label={title || 'Panel'} className={`relative ${WIDTHS[width] || WIDTHS.md} flex h-[100dvh] w-[min(100vw,42rem)] flex-col border-l divide-hairline bg-app shadow-[var(--imp-elevation-3)]`}>
